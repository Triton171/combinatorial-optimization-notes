...
Let $G$ be a graph, $u: E(G)\to\bR_{\geq0}$. Find $\emptyset\subsetneq X
\subsetneq V(G)$ minimizing $u(\delta(X))$. One approach: ${\abs{V(G)}\choose 2}$
$s$-$t$-cut computations (this can clearly be reduced to $\abs{V(G)}-1$
by fixing $s$).

\begin{definition}
	For $s,t\in V(G)$, denote by $\lambda_{s,t}$ the minimum capacity of an
