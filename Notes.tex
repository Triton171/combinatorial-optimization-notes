...
\begin{enumerate}[label=\alph*)]
	...
	\item The set $\set{\set{x,\mu(x)}\cup\set{x,\varphi(x)}\ |\ x\in V(G)
	, \varphi(\mu(x))=\mu(x)\land \varphi(x)\neq x} \cup \set{ \set{x,\varphi(x)}
	\cup\set{x,\mu(x)}\ 
	|\ \varphi(x)\neq x\land \varphi(\mu(x))\neq\mu(x)}$ forms the edge set of
	a special blossom forest
\end{enumerate}
\end{lemma}
