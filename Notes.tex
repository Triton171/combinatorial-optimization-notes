...
\section{Splitting-Off Lemma}

\begin{lemma}[Lovász]
	Let $G$ be a (multi-)graph with $V(G)=V\dot\cup\set{s}$ with $\abs{\delta(s)}$
	even and $k\geq2$ such that $\abs{\delta(U)}\geq k$ for all $\emptyset
	\neq U\subsetneq V$.
	Then $\forall\set{s,t}\in E:\ \exists u\in\Gamma(s)$ such that
	\[G'\coloneqq G-\set{s,t}-\set{s,u}+\set{u,t}\]
	satisfies $\abs{\delta_{G'}(U)}\geq k$ for all $\emptyset\subsetneq U\subsetneq V$.
\end{lemma}
